import { Client, Intents, to } from "../mod.ts";
import { Interaction } from "../src/structures/interaction.ts";
import { InteractionResponseEnum } from "../src/types/commands.ts";
...
(async () => {
    ...
    client.events.$attach(to("READY"), async () => {
    ...
    client.events.$attach(to("INTERACTION_CREATE"), async (interaction: Interaction) => {
